...
def decompose(
    ...
    """    
    Decomposition of extracted features using DBSCAN clustering.
    
    Algorithm: 
        - Loads the features
        - Creates the clusters based on the epsilon and the minimum points decided by the user
        - Each cluser then corresponds in its own data subfolder
    ...
    params:
        ...
        <int> minpoints: Number of minimum points
        <int> epsilon : Number of the epsilon 
    """
    ...
    #DBSCAN 
    db = DBSCAN(eps = epsilon, min_samples= minpoints ,random_state=0)
